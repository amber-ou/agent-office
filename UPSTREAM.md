...
| `.github/workflows/ci.yml` | 新增 `Domain Tests` / `Storage Tests` 兩個 step，並把兩者加進 summary 表與 Final Gate         | 取 union；**務必確認 Final Gate 的迴圈仍包含 `DOMAIN_TEST STORAGE_TEST`**，否則新測試會變成不擋 build 的 advisory |
